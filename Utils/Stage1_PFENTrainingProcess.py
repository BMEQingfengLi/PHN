...
def training(data_rootdir,
    ...
    # define model
    model = PFEN(out_channel=label_amount)
    model.cuda()
    model = nn.DataParallel(model)
    if os.path.isfile(pretrained_modelpth):
    ...
    for epoch in range(1, epoch_num):
        ...
        plt.clf()
        np.save(os.path.join(modelandresult_savepth, 'average_patch_loss.npy'),
                np.array(average_patch_loss_list))
        np.save(os.path.join(modelandresult_savepth, 'average_patch_testloss.npy'),
...
if __name__ == '__main__':
    ...
    pair = 'HCOthers'
    pretrained_modelpth = ""
    data_rootdir = '/HuaweiData/sharehome/yxpt/homeDocument/Documents/DL_trainingdata/20220809_PHN_Data/SMHC'
    origdatarootpth = r'./Files/'
    for fold in [1, 2, 3, 4, 5]:
        modelandresult_savepth = r'./Save_stage1/%s_fold%d' % (pair, fold)
        label_amount = 2
        epoch_num = 500
        radius = 24
        main(data_rootdir,
